...
import com.softtek.excepciones.ModeloNotFoundException;
...
public class PacienteControlador {
	...
	@GetMapping("/{id}")
	public ResponseEntity<PacienteDTO> listarUno(@PathVariable("id") Integer id) throws Exception {
		Paciente objeto = servicio.listarUno(id);
		if (objeto == null) {
	...
	public EntityModel<PacienteDTO> listarHateoas(@PathVariable("id") Integer id) throws Exception {
		Paciente obj = servicio.listarUno(id);
		if (obj == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO -> " + id);
		}
		...
		WebMvcLinkBuilder link1 = linkTo(methodOn(this.getClass()).listarUno(id));
...
//	}
	...
	public ResponseEntity<PacienteDTO> modificar(@Valid @RequestBody PacienteDTO p) throws Exception {
		...
		Paciente pacienteConsultado = servicio.listarUno(pacienteRequest.getIdPaciente());
	...
	public ResponseEntity<Void> eliminar(@PathVariable Integer id) throws Exception {
		Paciente objeto = servicio.listarUno(id);
		if (objeto == null) {
			throw new ModeloNotFoundException("ID NO ENCONTRADO " + id + " AL BORRAR");
		}
